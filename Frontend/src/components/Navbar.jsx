...
export default function Navbar({ shouldStick = true, isScrolled = false }) {
  ...
  return (
    <nav className={`${scrolledState ? 'shadow-md' : ''} relative bg-transparent`}>
      <div 
        className={`
          ${scrolledState 
                  ...
                  <div>
                    <button
                      className={`font-semibold font-inter text-sm lg:text-base flex items-center ${
                        activeLink === link.name || link.subLinks?.some(sub => activeLink === sub.name) 
                          ? 'text-[#F800EA]' 
                          : 'text-[#21001E]'
                      }`}
                    ...
                    >
                      {link.name}
                      <svg 
                        className={`ml-1 w-4 h-4 transition-transform ${isProgramsOpen ? 'rotate-180' : ''}`}
                        fill="none" 
                        stroke="currentColor" 
                        viewBox="0 0 24 24" 
                          ...
                          {link.subLinks.map((subLink) => (
                            <button
                              ...
                              className={`block px-4 py-2 text-sm w-full text-left ${
                                activeLink === subLink.name ? 'text-[#F800EA] bg-[#FFE6FD]' : 'text-[#21001E] hover:bg-[#FFE6FD]/50'
                              }`}
                          ...
                          {link.subLinks.map((subLink) => (
                            <button
                              ...
                              className={`block py-1 text-sm w-full text-left ${
                                activeLink === subLink.name ? 'text-[#F800EA]' : 'text-[#21001E]'
                              }`}
                              onClick={() => navigateTo(subLink.path)}
                            >
                              {subLink.name}
